 ...
 *
 * The copy constructor and assignment operators should throw a
 * BoundedCopyException if the operations fail (as they in some cases should).
 * When the copying fails, only the exception should be thrown and no changes
 * should be made to the existing pointer.
 *
...
class BoundedPtr {
 public:
  // default constructor
  /**
   * \brief Construct a new Bounded Ptr object with null pointer and use_
   * initialized with "nullptr" string.
   *
   */
  BoundedPtr() : raw_pointer_(nullptr), ref_count_(nullptr), use_("nullptr") {}
