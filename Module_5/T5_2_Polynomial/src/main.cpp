...
int main() {
  ...
  {
    Poly p1;
    std::cout << "Default-constructed polynomial: " << p1 << "(should be empty)"
              << std::endl;
    p1[0] = -5;
    p1[2] = 20;
    p1[1] += 10;
    const Poly& pref = p1;
    std::cout << "After modifications: " << pref
              << ", should be\n                     20x2+10x1-5x0" << std::endl;
    Poly p2 = -pref;
    std::cout << "After negation: " << p2
              << ", should be\n                -20x2-10x1+5x0" << std::endl;
  }
  std::cout << "TEST 2:" << std::endl;
  {
    ...
    if (iss >> p1 >> p2) {
      std::cout << "p1: " << p1 << ", should be\n    20x2+10x1+5x0"
                << std::endl;
      std::cout << "p2: " << p2
                << ", should be\n    3x4+4x2+2x-2 (note the negative exp)"
                << std::endl;
    } else
      std::cout << "Reading from stream failed (shouldn't have failed)"
                << std::endl;
    const Poly& p1ref = p1;
    const Poly& p2ref = p2;
    std::cout << "p1 + p2: " << p1ref + p2ref
